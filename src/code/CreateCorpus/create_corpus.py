...
LOG_FILE = current_working_directory+"\\logs\\corpus.log"
OUT_FILE = current_working_directory+"\\..\\..\\docfile\\corpus.txt"
...
# Keywords
keywords = json.load(open(current_working_directory +
                     "\\docfile\\"+args.themes, encoding="utf8"))


def filt_crit(x, kw_list): return all(x)
...
for i, fname in enumerate(tqdm(glob.glob(current_working_directory+"\\..\\..\\docfile\\" + f"{args.data}\\*.txt"))):
    nb_docs += 1
    doc_occurrences[i] = {}
    ...
    try:
        ...
        for topic, kw_list in keywords.items():
            if filt_crit([kw for kw in kw_list if kw in contents], kw_list):
                ...
                doc_occurrences[i][topic] = sum(
                    [contents.count(x) for x in kw_list])
